...
| **Phase 2.6**: i18n完全化（管理系・認証系） | 🚧 **進行中** | 10/18 | 56% |
...
**🚧 Phase 2.6 進行中: i18n完全化（管理系・認証系ページ対応）** - 56%完了
- ✅ Task 1完了: 管理系ページi18n対応（組織設定、メンバー管理、個人設定）
- 🎯 目標: 全ページの多言語対応完了
- 📋 タスク: 4タスク中1完了（管理系✅、認証系⏸️、エラーページ⏸️、メタデータ⏸️）
- ⏱️ 工数: 10/18時間完了（3営業日）
- 🔄 依存: Phase 2.5完了 ✅
- 📊 対象: 組織設定、メンバー管理、個人設定、ログイン、エラーページ等
...
## 🌐 Phase 2.6: i18n完全化（管理系・認証系ページ対応） 🚧 56%完了
...
1. **認証レイアウト** (`app/[locale]/(auth)/layout.tsx`)
   - ハードコーディング: 2箇所
   - メタデータ（タイトル、ディスクリプション）
   - 工数: 0.5時間

2. **ログインページ** (`app/[locale]/(auth)/login/page.tsx`)
   - ハードコーディング: 1箇所 + フォーム要素
   - 工数: 1.5時間
   - 内容:
     - ページタイトル
     - フォームラベル（メール、パスワード）
...
3. **サインアップページ** (未実装の場合はスキップ)
   - 工数: 1.5時間
   - 内容:
     - ページタイトル
     - フォームラベル（名前、メール、パスワード）
     - 利用規約リンク
     - ボタン（サインアップ）

4. **パスワードリセットページ** (未実装の場合はスキップ)
   - 工数: 0.5時間
...
2. **ロケールファイル更新**
   - `locales/ja/common.json` に認証関連の翻訳追加
   - `locales/en/common.json` に対応する英語翻訳追加

3. **コンポーネント更新**
   - `useTranslations('auth')` の導入
   - フォームラベル、ボタンの多言語化

**成果物**:
- `locales/ja/common.json` (auth セクション追加)
- `locales/en/common.json` (auth セクション追加)
- 認証関連ページのコンポーネント更新

---

### ⏸️ Task 3: エラーページi18n対応 待機中
...
2. **グローバルエラーページ** (`app/global-error.tsx`)
   - ハードコーディング: 1箇所
   - 工数: 0.5時間
   - 内容:
     - エラータイトル「システムエラー」
...
3. **404ページ** (`app/not-found.tsx` - 作成が必要な場合)
   - 工数: 1時間
   - 内容:
     - タイトル「ページが見つかりません」
     - 説明文
...
2. **ロケールファイル更新**
   - `locales/ja/errors.json` にページエラー追加
   - `locales/en/errors.json` に対応する英語翻訳追加

3. **エラーページ更新**
   - error.tsx、global-error.tsx の多言語化
   - not-found.tsx の作成（必要な場合）

**注意事項**:
- error.tsx、global-error.tsx は特殊なファイルで、`useTranslations`が使えない場合があります
- その場合は、クライアントコンポーネントとして実装するか、localeをpropsで受け取る設計にします

**成果物**:
- `locales/ja/errors.json` 更新
- `locales/en/errors.json` 更新
- エラーページのコンポーネント更新
