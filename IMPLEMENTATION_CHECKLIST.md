...
1. **APIエラーメッセージ多言語化**（3h）
   - エラーコード → 多言語メッセージマッピング
   - HTTP status code対応（400, 401, 403, 404, 500, etc.）
   - API固有エラー（ValidationError, AuthError, etc.）
   - エラーメッセージフォーマット統一
...
**実装ファイル**:
- `lib/messages/error-mapper.ts` - エラーコードマッピング
- `lib/messages/validation.ts` - Zod多言語化
