...
import { canAccessChildOrganizations, canModifyHierarchy } from '@/lib/auth/permissions';
import { organizationMembers, organizations } from '@/lib/db/schema';
import type { DataSharingPolicy, Organization, OrganizationType } from '@/lib/db/schema';
...
export const hierarchyRouter = router({
   ...
   */
  getHierarchy: protectedProcedure.input(organizationIdSchema).query(async ({ ctx, input }) => {
   ...
   */
  getChildren: protectedProcedure.input(organizationIdSchema).query(async ({ ctx, input }) => {
    // Verify membership
    const membership = await ctx.db.query.organizationMembers.findFirst({
      where: and(
        eq(organizationMembers.organizationId, input.organizationId),
   ...
   */
  getDescendants: protectedProcedure.input(organizationIdSchema).query(async ({ ctx, input }) => {
    const membership = await ctx.db.query.organizationMembers.findFirst({
      where: and(
        eq(organizationMembers.organizationId, input.organizationId),
        eq(organizationMembers.userId, ctx.user.id)
   ...
   */
  getAncestors: protectedProcedure.input(organizationIdSchema).query(async ({ ctx, input }) => {
    const membership = await ctx.db.query.organizationMembers.findFirst({
      where: and(
        eq(organizationMembers.organizationId, input.organizationId),
        eq(organizationMembers.userId, ctx.user.id)
   ...
   */
  getGroupStats: protectedProcedure.input(organizationIdSchema).query(async ({ ctx, input }) => {
    const membership = await ctx.db.query.organizationMembers.findFirst({
      where: and(
        eq(organizationMembers.organizationId, input.organizationId),
        eq(organizationMembers.userId, ctx.user.id)
    ...
    const [orgCount, holdingCount, subsidiaryCount] = await Promise.all([
      ...
      ctx.db
        ...
        .from(organizations)
        .where(
          and(eq(organizations.groupId, groupId), eq(organizations.organizationType, 'holding'))
        ),
      ctx.db
        .select({ count: sql<number>`count(*)` })
        .from(organizations)
        .where(
          and(eq(organizations.groupId, groupId), eq(organizations.organizationType, 'subsidiary'))
        ),
    ]);
